...
App::App() : m_display(), m_queue(), m_timer(), m_kill(false), m_dirty(true), m_counter(0), m_grid()
{
}
...
int32_t App::initialize(const std::vector<std::string>& argList)
{
	...
	Allegro::Path basePath(al_get_standard_path(ALLEGRO_RESOURCES_PATH));
	if (basePath)
	{
	...
	this->m_display.reset(al_create_display(App::SCREEN_WIDTH, App::SCREEN_HEIGHT));
	if (!this->m_display)
	{
		std::cout << "failed" << std::endl;
		return -1;
	...
	Allegro::Bitmap icon(al_load_bitmap("data\\images\\icon.png"));
	if (icon)
	{
	...
	this->m_queue.reset(al_create_event_queue());
	if (!this->m_queue)
	{
		std::cout << "failed" << std::endl;
		return -1;
	...
	this->m_timer.reset(al_create_timer(1.0 / App::LOGIC_RATE));
	if (!this->m_timer)
	{
		std::cout << "failed" << std::endl;
		return -1;
	...
	if (this->m_grid.generateGrid(App::SCREEN_WIDTH, App::SCREEN_HEIGHT) < 0)
	{
		return -1;
	}
...
void App::shutdown()
{
	...
	if (Cell::freeSprites())
	{
		std::cout << "X/O Sprites Destroyed" << std::endl;
	}
